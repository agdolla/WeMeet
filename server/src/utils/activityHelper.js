...
module.exports = class ActivityHelper {
    ...
    resolveActivityItem(activityItem) {
        return new Promise((resolve, reject) => {
            ...
            this.serverHelper.resolveUserObjects(userList, function(
                ...
                activityItem.participants = activityItem.participants.map(
                    id => userMap[id]
                );
                activityItem.likeCounter = activityItem.likeCounter.map(
                    id => userMap[id]
                );
    ...
    getActivityItemComments(activityItemId, date) {
        return this.database
            .collection("activityItemComments")
            .aggregateAsync([
    ...
    getActivityChatMessages(activityItemId, date) {
        return this.database
            .collection("activityItemChatMessages")
            .aggregateAsync([
                { $match: { _id: activityItemId } },
                { $unwind: "$messages" },
    ...
    createActivity(data, callback) {
        ...
        this.database
            ...
            .then(() => {
                return this.database
                    .collection("users")
                    .findOneAsync({ _id: new ObjectID(data.author) });
            })
            .then(userData => {
                this.database.collection("activities").updateOne(
                    { _id: userData.activity },
                    {
                        $push: {
                            contents: {
    ...
    getJoinedActivity(userId) {
        return this.database
            .collection("activityItems")
            .findAsync({
                $or: [{ participants: { $in: [userId] } }, { author: userId }],
