        ...
        return new Promise((resolve, reject) => {
            this.database
                ...
                .then(cursor => {
                    return cursor
                        .limit(5)
                        .sort({ "contents.postDate": -1 })
                        .toArrayAsync();
                })
                .then(collection => {
                    var processNextFeedItem = i => {
                        this.resolvePostItem(collection[i]).then(postItem => {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.serverHelper.resolveUserObjects(userList, (err, userMap) => {
                if (err) reject(err);
                    ...
                    postFeedItem.likeCounter = postFeedItem.likeCounter.map(
                        id => userMap[id]
                    );
                    postFeedItem.contents.author =
                        userMap[postFeedItem.contents.author];
            ...
            this.database
                ...
                .then(feedData => {
                    ...
                    var processNextFeedItem = i => {
                        ...
                        this.getPostFeedItem(feedData.contents[i]).then(
                            feedItem => {
