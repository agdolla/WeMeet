...
export default class Notification extends React.Component {
    ...
    getData() {
        getNotificationData(this.props.user._id).then(response => {
    ...
    handleDelete = id => {
        deleteNotification(id, this.props.user._id).then(response => {
            let notificationData = response.data;
            var FR = [];
            var AN = [];
    ...
    handleFriendAccept = (id, user) => {
        acceptFriendRequest(id, this.props.user._id).then(() => {
            this.getData();
    ...
    handleActivityAccept = (notificationid, user) => {
        acceptActivityRequest(notificationid, this.props.user._id).then(() => {
            this.getData();
            socket.emit("accept notification", {
                target: user
            });
